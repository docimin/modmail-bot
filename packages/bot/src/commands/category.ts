...
const command: BotCommand = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s
        ...
        .addStringOption((o) =>
          o.setName("description").setDescription("What this category is for"),
        )
        .addStringOption((o) =>
          o.setName("emoji").setDescription("Emoji shown next to the category"),
        ),
    )
    .addSubcommand((s) =>
      s
  ...
  async execute({ interaction, services }) {
    ...
    if (sub === "add") {
      ...
      await interaction.reply({
        content: `✅ Category \`${parsed.data.name}\` created.`,
        flags: MessageFlags.Ephemeral,
      });
      return;
    }
    ...
    if (sub === "remove") {
      ...
      await interaction.reply({
        content: `🗑️ Category \`${cat.name}\` removed.`,
        flags: MessageFlags.Ephemeral,
      });
      return;
    }
      ...
      .setDescription(
        rows.length
          ? rows
              .map(
                (c) =>
                  `${c.emoji ? `${c.emoji} ` : ""}**${c.name}**${c.description ? ` — ${c.description}` : ""}`,
              )
              .join("\n")
              .slice(0, 4000)
