  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s
        ...
        .addStringOption((o) => o.setName("content").setDescription("Snippet content").setRequired(true)),
    )
    .addSubcommand((s) =>
      s
        .setName("edit")
  ...
  async execute({ interaction, services, ticket }) {
    ...
    if (sub === "add" || sub === "edit") {
      ...
      await db
        ...
        .values({ guildId, name: parsed.data.name, content: parsed.data.content, createdById: interaction.user.id })
        .onConflictDoUpdate({
          target: [schema.snippets.guildId, schema.snippets.name],
          set: { content: parsed.data.content, updatedAt: new Date() },
        });
      await interaction.reply({ content: `✅ Snippet \`${parsed.data.name}\` saved.`, flags: MessageFlags.Ephemeral });
      return;
    }

    if (sub === "remove") {
      const name = interaction.options.getString("name", true);
      await db.delete(schema.snippets).where(and(eq(schema.snippets.guildId, guildId), eq(schema.snippets.name, name)));
      await interaction.reply({ content: `🗑️ Snippet \`${name}\` removed.`, flags: MessageFlags.Ephemeral });
      return;
    }

    if (sub === "list") {
      const rows = await db.query.snippets.findMany({ where: eq(schema.snippets.guildId, guildId) });
      const embed = new EmbedBuilder()
        .setColor(0x5865f2)
        .setTitle(`Snippets (${rows.length})`)
        .setDescription(
          rows.length
            ? rows.map((s) => `**${s.name}** — ${s.content.slice(0, 60)}`).join("\n").slice(0, 4000)
    ...
    // send
    if (!ticket) {
      await interaction.reply({ content: "Use `/snippet send` inside a ticket channel.", flags: MessageFlags.Ephemeral });
      return;
    }
    const name = interaction.options.getString("name", true);
