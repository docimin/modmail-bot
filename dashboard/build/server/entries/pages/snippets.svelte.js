...
/* empty css                                                           */var snippets_svelte_svelte_type_style_lang = "";
const css = {
...
const Snippets = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${"container svelte-1b09m5s"}"><div class="${"header svelte-1b09m5s"}"><span class="${"svelte-1b09m5s"}">Snippets</span>
   <form class="${"search " + escape(snippets ? "" : "disabled") + " svelte-1b09m5s"}"><input type="${"text"}" placeholder="${"Snippet name / content"}" ${!snippets ? "disabled" : ""} class="${"svelte-1b09m5s"}"${add_attribute("value", snippetsFilter, 0)}>
  ...
  <div class="${"tickets svelte-1b09m5s"}">${snippets ? `<div class="${"ticket columns-template svelte-1b09m5s"}">${each(columns, (column) => {
    return `<div class="${"column " + escape(column.id) + " svelte-1b09m5s"}" style="${"width: " + escape(column.width) + "%; min-width: " + escape(column.minwidth) + "px;"}">${escape(column.label)}</div>`;
  })}</div>
    ${snippets.length ? each(snippets, (user) => {
    return `<button class="${"ticket svelte-1b09m5s"}">${each(columns, (column) => {
      return `<div class="${"column " + escape(column.id) + " svelte-1b09m5s"}" style="${"width: " + escape(column.width) + "%; min-width: " + escape(column.minwidth) + "px;"}"><!-- HTML_TAG_START -->${column.get(user)}<!-- HTML_TAG_END --></div>`;
    })}
