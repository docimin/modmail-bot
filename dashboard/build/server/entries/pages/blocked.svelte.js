import { c as create_ssr_component, a as subscribe, e as escape, d as add_attribute, v as validate_component, f as each } from "../../chunks/index-aa015feb.js";
import { R as Rolling } from "../../chunks/Rolling-99d5e5b9.js";
import { I as InfiniteScroll } from "../../chunks/InfiniteScroll-5a38fd30.js";
import { u as updateSite, p as popupData } from "../../chunks/Popup-516f6855.js";
/* empty css                                                           */var blocked_svelte_svelte_type_style_lang = "";
const css = {
...
const Blocked = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${"container svelte-1m23zr9"}"><div class="${"header svelte-1m23zr9"}"><span class="${"svelte-1m23zr9"}">Blockierte User</span>
   <form class="${"search " + escape(blocked ? "" : "disabled") + " svelte-1m23zr9"}"><input type="${"text"}" placeholder="${"User ID"}" ${!blocked ? "disabled" : ""} class="${"svelte-1m23zr9"}"${add_attribute("value", blockedFilter, 0)}>
  ...
  <div class="${"tickets svelte-1m23zr9"}">${blocked ? `<div class="${"ticket columns-template svelte-1m23zr9"}">${each(columns, (column) => {
    return `<div class="${"column " + escape(column.id) + " svelte-1m23zr9"}" style="${"width: " + escape(column.width) + "%; min-width: " + escape(column.minwidth) + "px;"}">${escape(column.label)}</div>`;
  })}</div>
    ${blocked.length ? each(blocked, (user) => {
    return `<button class="${"ticket svelte-1m23zr9"}">${each(columns, (column) => {
      return `<div class="${"column " + escape(column.id) + " svelte-1m23zr9"}" style="${"width: " + escape(column.width) + "%; min-width: " + escape(column.minwidth) + "px;"}"><!-- HTML_TAG_START -->${column.get(user, users)}<!-- HTML_TAG_END --></div>`;
    })}
